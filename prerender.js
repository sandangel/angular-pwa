...
async function main() {
  ...
  const index = (await readFile(
    join(process.cwd(), 'dist/apps/admin-mobile/browser', 'index.html')
  )).toString();

  // Serving the static files.
  app.get('*.*', express.static(join(process.cwd(), 'dist/apps/admin-mobile/browser')));
  ...
  do {
    ...
    const file = join(process.cwd(), 'dist/apps/admin-mobile/browser', (p || 'index') + '.html');
    ...
    PAGES = difference(
      uniq(
        PAGES.concat(result.match(/href="\/[\/\w\d\-]*"/g).map(s => s.match(/\/([\/\w\d\-]*)/)[1]))
      ),
      RENDERED_PAGES
    );
